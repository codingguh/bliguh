    ...
    try {
      ...
      if (user.value.id.isEmpty) {
        if (userCredential != null) {
          ...
          storage.write('photo', userCredential.user!.photoURL);

          // Update user model
          // user.value.update((val) {
          //   val!.id = userCredential.user!.uid;
          //   val.firstName = nameParts[0];
          //   val.lastName =
          //       nameParts.length > 1 ? nameParts.sublist(1).join(' ') : '';
          //   val.userName = username;
          //   val.email = userCredential.user!.email ?? '';
          //   val.phoneNumber = userCredential.user!.phoneNumber ?? '';
          //   val.profilePicture = userCredential.user!.photoURL ?? '';
          // });
          ...
          //save user data
          await userRepository.saveUserRecord(user.value);
        } else {
          print('gdsgsdgsdg gagal');
        }
      }
    } catch (e) {
      Loaders.warningSnackBar(
          ...
          message:
              '$e Something went wrong while saving your information. You can resave your data in your profile ');
    }
  }
...
// Convert CroppedFile to File
      ...
      await uploadTask.whenComplete(() async {
        ...
        final photoProfile = storage.write('photoProfile', downloadUrl);

        user.refresh();

        print('download url $downloadUrl--photo $photoProfile');
        Loaders.successSnackBar(
